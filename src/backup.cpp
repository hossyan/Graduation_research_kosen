...
const float target_angle_inc = 5.0;
...
// ピンの定義
const int LSW1Pin = 26;  
const int LSW2Pin = 36;  
...
void setup(){
  ...
  // pinMode(LSW1Pin, INPUT_PULLUP);
  // pinMode(LSW2Pin, INPUT_PULLUP);
...
void tire_controll() {
  if(balance_mode){
    target_angle = - 14 - (PS4.LStickY() * target_angle_inc /  joystick_max);
...
void target_angle_calc(){
  bool current_button[2] = {
    PS4.Right(),
    PS4.Left()
  };

  if(current_button[0] && !prev_Right){
    target_angle -= 0.1;
  }
  if(current_button[1] && !prev_Left){
    target_angle += 0.1;
  }

  prev_Right = current_button[0];
  prev_Left = current_button[1];
}
...
void self_balance_controll(){
  ...
  if(balance_mode){
    if(flag){
      ...
      //cosで値の増幅
      float rad_roll = (target_angle - roll) * (M_PI / 180);
      float cos_roll = cos(rad_roll);

      float output_adjusted = output / cos_roll;

      //値のクランプ
      if(output_adjusted >= 1.0){
        output_adjusted = 1.0;
      }else if(output_adjusted <= -1.0){
        output_adjusted = -1.0;
      }

      motor_out[0] = - output_adjusted * 16000;
      motor_out[1] =   output_adjusted * 16000;
    } 
  }
}
...
void display_update(){
  ...
  M5.Lcd.printf("kp:%14.7f\n",kp);
  M5.Lcd.printf("kd:%14.7f\n",kd);
  M5.Lcd.printf("ki:%14.7f\n",ki);
  M5.Lcd.printf("target_angle:%14.7f\n",target_angle);

  M5.Lcd.printf("roll:%12.5f\n",roll);
  M5.Lcd.printf("R_output:%14d\n",motor_out[0]);
  M5.Lcd.printf("L_output:%14d\n",motor_out[1]);

  if(balance_mode){
    M5.Lcd.printf("Mode : busy");
  }else if(!balance_mode){
    M5.Lcd.printf("Mode : free");
  }
}